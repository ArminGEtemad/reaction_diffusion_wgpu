...
fn fs_main(in : VSOut) -> @location(0) vec4<f32> {
    ...
    if (u_display.split_screen == 0u) {
        let col = textureSampleLevel(rd1_texture, rd_sampler, uv_sim, 0.0);
        return vec4<f32>(col.rgb, 1.0);
    }

    // split-screen: draw white boundary
    let boundary_width = 0.001;
    if (abs(uv_sim.x - 0.5) < boundary_width) {
        return vec4<f32>(1.0, 1.0, 1.0, 1.0);
    }

    if (uv_sim.x < 0.5) {
        let uv_left = vec2<f32>(uv_sim.x * 2.0, uv_sim.y);
        let col = textureSampleLevel(rd1_texture, rd_sampler, uv_left, 0.0);
        return vec4<f32>(col.rgb, 1.0);
    } else {
        let uv_right = vec2<f32>((uv_sim.x - 0.5) * 2.0, uv_sim.y);
